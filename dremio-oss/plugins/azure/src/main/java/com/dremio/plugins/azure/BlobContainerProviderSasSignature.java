 ...
 */
public class BlobContainerProviderSasSignature extends BaseBlobContainerProvider {

  private final AzureSasSignatureCredentials credentials;
  private volatile CloudBlobClient cloudBlobClient;

  public BlobContainerProviderSasSignature(
      AzureStorageFileSystem parent,
      URI connection,
      String account,
      String[] containers,
      AzureSasSignatureCredentials credentials) {
  ...
  protected CloudBlobClient getCloudBlobClient() {
    cloudBlobClient =
        new CloudBlobClient(getConnection(), credentials.getStorageCredentials(getAccount()));
