 ...
 * @param <U> Second type
 */
public interface PairList<T, U> extends List<Entry<T, U>> {
   ...
   *
   * <p>Changes to the backing list will be reflected in the PairList. If the backing list is
   * immutable, this PairList will be also.
   */
  static <T, U> PairList<T, U> backedBy(
      List<@org.checkerframework.checker.nullness.qual.Nullable Object> list) {
    return new PairLists.MutablePairList<>(list);
  }
  ...
  /**
   * Creates an {@link ImmutableMap} whose entries are the pairs in this list. Throws if keys are
   * not unique.
   */
