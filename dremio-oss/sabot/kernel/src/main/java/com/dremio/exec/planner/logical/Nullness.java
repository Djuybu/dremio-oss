...
/**
 * The methods in this class allow to cast nullable reference to a non-nullable one. This is an
 * internal class, and it is not meant to be used as a public API.
 *
...
public class Nullness {
   ...
   * </code></pre>
   *
   * <p>That signature allows you to use {@code Wrapper} with both nullable or non-nullable types:
   ...
   * </code></pre>
   *
   * <p>The issue is checkerframework does not permit that because {@code T} has unknown
   * nullability, so the following needs to be used:
   *
   * <pre><code>
  ...
  /**
   * Allows you to treat an uninitialized or under-initialization object as initialized with no
   * assertions.
   *
   * @param <T> The type of the reference
