...
public class PairLists {
    ...
    switch (list.size()) {
      ...
      case 2:
        return new SingletonImmutablePairList<>(
            castNonNull((T) list.get(0)), castNonNull((U) list.get(1)));
      default:
  ...
  static @NonNull Object[] checkElementsNotNull(
      @org.checkerframework.checker.nullness.qual.Nullable Object... elements) {
  ...
  private static Object[] castNonNullArray(Object[] elements) {
    return elements;
  }

  static void checkElementNotNull(
      int i, @org.checkerframework.checker.nullness.qual.Nullable Object element) {
    if (element == null) {
   ...
   * @param <U> Second type
   */
  abstract static class AbstractPairList<T, U> extends AbstractList<Entry<T, U>>
      implements PairList<T, U> {
    /** Returns a list containing the alternating left and right elements of each pair. */
    abstract List<@org.checkerframework.checker.nullness.qual.Nullable Object> backingList();

    @Override
    public abstract PairList<T, U> subList(int fromIndex, int toIndex);
  ...
  static class MutablePairList<T, U> extends AbstractPairList<T, U> {
    final List<@org.checkerframework.checker.nullness.qual.Nullable Object> list;

    MutablePairList(List<@org.checkerframework.checker.nullness.qual.Nullable Object> list) {
      this.list = list;
    }

    @Override
    List<@org.checkerframework.checker.nullness.qual.Nullable Object> backingList() {
      return list;
    }
    ...
    @Override
    public Map.Entry<T, U> set(
        int index, Map.@org.checkerframework.checker.nullness.qual.Nullable Entry<T, U> entry) {
      if (entry == null) {
    ...
    private List<Object> castNonNullList(List<Object> list) {
      return list;
    }
    ...
    public void reverse() {
      for (int i = 0, j = list.size() - 2; i < j; ) {
        @org.checkerframework.checker.nullness.qual.Nullable Object o = list.get(i);
  ...
  static class EmptyImmutablePairList<T, U> extends AbstractPairList<T, U>
    ...
    @Override
    public ImmutablePairList<T, U> subList(int fromIndex, int toIndex) {
  ...
  static class SingletonImmutablePairList<T, U> extends AbstractPairList<T, U>
    ...
    public ImmutablePairList<T, U> subList(int fromIndex, int toIndex) {
      subListRangeCheck(fromIndex, toIndex, size());
      return fromIndex > toIndex ? this : ImmutablePairList.of();
    }
   ...
   */
  abstract static class RandomAccessList<E> extends AbstractList<E> implements RandomAccess {}
  ...
  static class ArrayImmutablePairList<T, U> extends AbstractPairList<T, U>
     ...
     *
     * <p>Does not copy the {@code elements} array. Assumes that the caller has made a copy, and
     * will never modify the contents.
     *
     * <p>Assumes that {@code elements} is not null, but checks that none of its elements are null.
     */
    ArrayImmutablePairList(@org.checkerframework.checker.nullness.qual.Nullable Object[] elements) {
    ...
    public ImmutablePairList<T, U> subList(int fromIndex, int toIndex) {
      ...
      switch (toIndex - fromIndex) {
        ...
        case 2:
          return new SingletonImmutablePairList<>(
              (T) elements[fromIndex * 2], (U) elements[fromIndex * 2 + 1]);
        default:
          return new ArrayImmutablePairList<>(
              Arrays.copyOfRange(elements, fromIndex * 2, toIndex * 2 - fromIndex * 2));
      }
    }
