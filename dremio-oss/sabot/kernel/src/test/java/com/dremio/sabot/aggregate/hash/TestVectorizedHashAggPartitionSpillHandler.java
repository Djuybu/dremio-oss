...
import com.dremio.sabot.op.common.ht2.TestVarBinaryPivot;
 ...
 */
public class TestVectorizedHashAggPartitionSpillHandler extends BaseTestOperator {
  ...
  private int TARGET_BATCH_SIZE = 3968;
  ...
  private void testHelper(
    ...
    for (int i = 0; i < numPartitions; i++) {
      ...
      HashTable sourceHashTable =
          HashTable.getInstanceForAggregate(sabotConfig, optionManager, hashTableCreateArgs);
      ...
      int maxHashTableBatchSize = HASHAGG_MAX_BATCH_SIZE.getDefault().getNumVal().intValue();
    ...
    final int variableWidthDataRowSize =
        ((estimatedVariableWidthKeySize + HashTable.VAR_LENGTH_SIZE) * pivot.getVariableCount())
            + HashTable.VAR_LENGTH_SIZE;
    ...
    try {
      ...
      try (final PartitionToLoadSpilledData partitionToLoadSpilledData =
        ...
        /* spill a partition */
        VectorizedHashAggPartition victimPartition1 =
            partitionSpillHandler.chooseVictimPartition(false);
        ...
        VectorizedHashAggPartition victimPartition2 =
            partitionSpillHandler.chooseVictimPartition(false);
