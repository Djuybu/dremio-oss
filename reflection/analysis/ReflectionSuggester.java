...
public class ReflectionSuggester {
  ...
  private List<ReflectionDetails> getAggReflections(TableStats tableStats) {
    try {
      AutonomousReflectionClient.AIResponse aiResponse = AutonomousReflectionClient.getRecommendations(datasetConfig);
      if (aiResponse != null && (aiResponse.dimensions != null || aiResponse.measures != null)) {
        
        List<ReflectionDimensionField> dimFields = new java.util.ArrayList<>();
        if (aiResponse.dimensions != null) {
          for (String dim : aiResponse.dimensions) {
        ...
        if (aiResponse.measures != null) {
          ...
          for (String meas : aiResponse.measures) {
            ReflectionMeasureField mf = new ReflectionMeasureField(meas);
            if (statsMap.containsKey(meas)) {
              mf.setMeasureTypeList(ReflectionValidator.getDefaultMeasures(statsMap.get(meas).getField().getTypeFamily()));
            } else {
              mf.setMeasureTypeList(com.google.common.collect.ImmutableList.of(com.dremio.service.reflection.proto.MeasureType.SUM, com.dremio.service.reflection.proto.MeasureType.COUNT));
            }
        ...
        logger.info("Using Autonomous AI generated reflections: Dimensions: {}, Measures: {}", 
                    aiResponse.dimensions, aiResponse.measures);
